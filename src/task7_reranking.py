...
def rerank_cross_encoder(
    ...
    """
    Rerank candidates sử dụng Jina Reranker v2 (multilingual) qua API.
...
def rerank_mmr(
    ...
    results = []
    for i in selected:
        item = {**candidates[i], "score": cosine_sim(query_embedding, candidates[i]["embedding"])}
        results.append(item)
    return results
...
def rerank(
    ...
    if method == "cross_encoder":
        ...
        # MMR cần query_embedding + embedding trên từng candidate — gọi rerank_mmr()
        # trực tiếp với các vector đã embed sẵn, interface chung không tự embed hộ.
        raise NotImplementedError(
            "MMR cần query_embedding — gọi rerank_mmr(query_embedding, candidates, top_k) trực tiếp"
        )
    elif method == "rrf":
        # candidates ở đây thường đã là 1 list phẳng (vd sau khi Task 9 merge
        # semantic_search + lexical_search). RRF vẫn áp dụng được cho 1 list đơn —
        # chỉ gán lại score theo rank, thứ tự giữ nguyên. Muốn fuse nhiều list
        # (nhiều ranker) cùng lúc, gọi rerank_rrf([list1, list2, ...]) trực tiếp.
